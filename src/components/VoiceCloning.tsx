...
export function VoiceCloning() 
{
  ...
  const [selectedProfile, setSelectedProfile] = useKV<string | null>(
  ...
  const startRecording = async () => 
  {
    if (!profileName.trim()) 
    {
      toast.error(
        language === "fr"
          ? "Veuillez entrer un nom pour le profil vocal"
  ...
  const processRecording = async () => 
  {
    if (audioChunksRef.current.length === 0) 
    {
      setRecordingState("error");
      toast.error(
        language === "fr" ? "Aucun audio enregistré" : "No audio recorded",
      );
      return;
    }
  ...
  const deleteProfile = async (profileId: string) => 
  {
    ...
    if (profile?.mistralVoiceId && currentUserSettings.mistralApiKey) 
    {
      ...
      {
        await fetch(
          `https://api.mistral.ai/v1/audio/voices/${profile.mistralVoiceId}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${currentUserSettings.mistralApiKey}`,
    ...
    toast.success(
      language === "fr" ? "Profil vocal supprimé" : "Voice profile deleted",
    );
  };
  ...
  const userName =
    currentUserSettings.firstName || (language === "fr" ? "Marie" : "John");
  const sampleText =
    language === "fr"
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={
                      language === "fr" ? "Ex: Ma voix" : "Ex: My Voice"
                    }
                    className="mt-2"
                  />
                </div>
                          ...
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-muted-foreground">
                                ...
                                · {Math.round(profile.duration / 1000)}s
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                              ...
                              onClick={(e) => 
{
                              ...
                              title={
                                language === "fr"
                                  ? "Écouter l'enregistrement original"
                              ...
                              onClick={(e) => 
{
                              ...
                              title={
                                language === "fr"
                                  ? "Tester la voix clonée avec Mistral TTS"
                                  : "Test cloned voice with Mistral TTS"
                              }
                            >
                              <Waveform
                                size={16}
